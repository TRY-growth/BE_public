...
@Tag(name = "01")
public class Task2SubmitController {
    ...
    @Autowired
    public Task2SubmitController(ReportService reportService, Task2Repo task2Repo, Prompts prompts) {
    ...
    @Operation(summary = "Task2 제출 후 통합/상세 피드백 받기")
    public ResponseEntity<Report> submitTask2(@PathVariable String taskId, @RequestBody String submitAnswer) {
        Task2 task2 = task2Repo.findByTaskId(taskId);
        String passage = task2.getPassage();
        String discussion = task2.getDiscussion();
        String question = task2.getQuestion();

        // Reportt에 유저 답변 저장하기
        Report report = new Report();
        report.setTaskId(taskId);
        report.setTaskType("task2");
        ...
        String promptForTotalFeedback = "task 2 passage: " + passage + " task 2 discussion: " + discussion + " task 2 question: " + question + " task 2 user Answer: " + submitAnswer + prompts.task2TotalFeedback;
        ChatCompletionRequest chatCompletionRequest1 = new ChatCompletionRequest("gpt-4o", promptForTotalFeedback);

        // 사용자 답안 + 프롬프트 -> 모범 답안 얻기
        String promptForModifiedAnswer = prompts.task2GetModifiedAnswerFromGpt + " user answer: " + submitAnswer;
